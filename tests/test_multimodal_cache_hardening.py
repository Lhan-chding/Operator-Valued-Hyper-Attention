...
class MultimodalCacheHardeningTests(unittest.TestCase):
    ...
    def test_cache_validator_rejects_blank_failed_sample_reason(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (layout.root / "provenance" / "failed_samples_train.jsonl").write_text(
                json.dumps({"source_id": "failed-train", "split": "train", "reason": "   "}, sort_keys=True)
                + "\n"
            )
            _write_complete_checksums(layout.root)
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "failed_samples_train.jsonl line 1 required fields must be non-empty strings: reason",
            "\n".join(report.errors),
        )

    def test_cache_validator_rejects_unnormalized_failed_sample_source_id(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (layout.root / "provenance" / "failed_samples_train.jsonl").write_text(
                json.dumps({"source_id": " failed-train ", "split": "train", "reason": "download_failed"}, sort_keys=True)
                + "\n"
            )
            _write_complete_checksums(layout.root)
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "failed_samples_train.jsonl line 1 source_id must be a non-empty normalized source_id",
            "\n".join(report.errors),
        )
    ...
    def test_cache_validator_requires_original_split_provenance(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "sample_records_train.jsonl line 1 missing required keys: original_split",
            "\n".join(report.errors),
        )

    def test_cache_validator_requires_preprocessing_version_provenance(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "sample_records_train.jsonl line 1 missing required keys: preprocessing_version",
            "\n".join(report.errors),
        )

    def test_cache_validator_rejects_unnormalized_original_split_provenance(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "sample_records_train.jsonl line 1 original_split must be a non-empty normalized split provenance",
            "\n".join(report.errors),
        )
    ...
    def test_cache_validator_rejects_blank_data_card_tasks(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "data_card.json tasks must be a non-empty list of strings",
            "\n".join(report.errors),
        )
    ...
    def test_cache_validator_rejects_unnormalized_source_id_manifest_lines(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout

        with tempfile.TemporaryDirectory() as tmp:
            layout = MultimodalCacheLayout(Path(tmp), "refcoco", "v0.1")
            _write_minimal_cache(layout.root, train_ids=["train-source"], test_ids=["test-source"], mismatched_features=False)
            (layout.root / "provenance" / "source_ids_train.txt").write_text(" train-source \n")
            _write_complete_checksums(layout.root)
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "provenance/source_ids_train.txt line 1 must be a non-empty normalized source_id",
            "\n".join(report.errors),
        )
    ...
    def test_cache_validator_rejects_source_overlap_in_unvalidated_splits_manifest_entry(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "splits.json source_id appears in multiple splits: train-source (iid, train)",
            "\n".join(report.errors),
        )

    def test_cache_validator_rejects_non_list_unvalidated_splits_manifest_entry(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "splits.json iid must be a source_id list",
            "\n".join(report.errors),
        )

    def test_cache_validator_rejects_unnormalized_splits_manifest_key(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "splits.json split names must be non-empty normalized strings",
            "\n".join(report.errors),
        )

    def test_cache_validator_rejects_unnormalized_source_id_in_unvalidated_splits_manifest_entry(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "splits.json iid source_id entries must be non-empty normalized strings",
            "\n".join(report.errors),
        )
    ...
    def test_cache_validator_rejects_blank_feature_extractor_versions(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
                ...
                json.dumps(
                    {
                        ...
                        "region": "clip-region-v1",
                        "baselines": {
                            "cross_attention_transformer": {"text": "   ", "region": "clip-region-v1"},
                            "ovha_full": {"text": "   ", "region": "clip-region-v1"},
                        },
                    },
                    sort_keys=True,
                )
    ...
    def test_cache_validator_rejects_pseudo_label_unknown_source_split(self):
        from moat_ovha_torch.data.multimodal.cache_schema import MultimodalCacheLayout, validate_cache_layout
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_minimal_cache(layout.root, train_ids=["train-source"], test_ids=["test-source"], mismatched_features=False)
            (layout.root / "provenance" / "pseudo_label_versions.json").write_text(
                json.dumps({"generated_from_splits": ["holdout"], "version": "bad"}) + "\n"
            )
            _write_complete_checksums(layout.root)
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "pseudo_label_versions.json generated_from_splits contains unknown split: holdout",
            "\n".join(report.errors),
        )
