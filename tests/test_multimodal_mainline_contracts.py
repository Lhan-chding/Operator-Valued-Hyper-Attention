...
class MultimodalMainlineTorchContractTests(unittest.TestCase):
    ...
    def test_multimodal_ovha_ablation_variants_change_router_and_reliability_behavior(self):
        ...
        self.assertAlmostEqual(float(cato_only_output.diagnostics["router_load_by_candidate"]["CATO"]), 1.0, places=6)
        self.assertAlmostEqual(float(cato_only_output.diagnostics["router_load_by_candidate"]["LRIO"]), 0.0, places=6)
        self.assertIn("raw_router_load_by_candidate", cato_only_output.diagnostics)
        ...
        self.assertAlmostEqual(float(no_cato_output.diagnostics["router_load_by_candidate"]["CATO"]), 0.0, places=6)
        ...
        no_lrio_output = no_lrio(batch)
        no_spo_output = no_spo(batch)
        self.assertTrue(torch.allclose(no_lrio_output.router_weights[..., 2], torch.zeros(2, 5), atol=1e-6))
        self.assertTrue(torch.allclose(no_spo_output.router_weights[..., 1], torch.zeros(2, 5), atol=1e-6))
        self.assertAlmostEqual(float(no_lrio_output.diagnostics["router_load_by_candidate"]["LRIO"]), 0.0, places=6)
        self.assertAlmostEqual(float(no_spo_output.diagnostics["router_load_by_candidate"]["SPO"]), 0.0, places=6)
