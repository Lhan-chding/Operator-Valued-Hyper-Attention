...
class LocalKernelPrimitive(nn.Module):
    ...
    def forward(
        ...
        if params is not None and params.kernel_params and "lengthscale" in params.kernel_params:
            lengthscale = torch.nn.functional.softplus(params.kernel_params["lengthscale"]).unsqueeze(-2) + 1e-3
        kernel = torch.exp(-((target_q.unsqueeze(-2) - grid.unsqueeze(1)) ** 2) / (2.0 * lengthscale**2))
