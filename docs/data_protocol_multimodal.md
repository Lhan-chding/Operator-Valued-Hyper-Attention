...
Controlled candidate collapse gates must use the `true_learned` oracle matrix cell, meaning true router with learned adapter/candidate outputs, against the specialist threshold. The `learned_learned` cell belongs to full-model performance and must not be used to fail a candidate collapse gate because router failures are handled by the separate router gate.
...
Public statistics summaries must include every internal same-feature sanity probe and OVHA ablation required by the task, not just the single comparison baseline used in the main delta. Region-text reports must include text-only, region-only, concat fusion, CATO-only, `ovha_no_cato`, `ovha_no_rceo`, and `ovha_no_evidence_router`. Sentiment/emotion reports must include text-only, audio-only, vision-only, concat fusion, `spo_only`, `lrio_only`, `ovha_tanso_only`, `ovha_spo_lrio`, `ovha_lrio_tanso`, `ovha_all_candidates_exploratory`, `ovha_no_rceo`, and `ovha_with_evidence_router`. Do not list duplicate CMU primary variants such as `ovha_no_lrio` and `ovha_spo_tanso` as separate required rows; the admitted CMU primary is `SPO+TANSO`, while LRIO is evaluated through the admission/non-interference rows. External SOTA entries such as MDETR, GLIP, GroundingDINO, TFN/LMF, MulT, MISA, MAG-BERT, and Self-MM must be reported as `external_reference` or `external_reproduction` evidence in a separate table; they must not be listed as same-feature public-main baselines.

Every diagnostics row must include report-facing flat `adapter_params` keys `TLEO_lengthscale`, `SPO_temperature`, `LRIO_rank_entropy`, and `CATO_alignment_temperature`; implementations may also emit `adapter_params_detail` for nested per-candidate parameter means, but public gates consume the flat object. Every diagnostics row must include `candidate_diagnostics` with the Step 14.1 candidate-specific fields. `TLEO` requires `lengthscale`, `local_entropy`, `local_window_size`, and `candidate_loss`; `SPO` requires `prototype_entropy`, `top_prototype`, `prototype_temperature`, and `candidate_loss`; `LRIO` requires `rank_entropy`, `rank_top_k`, `pair_interaction_strength`, and `candidate_loss`; `CATO` requires `alignment_entropy`, `top_k_alignment`, `transport_marginal_error`, and `candidate_loss`; `RCEO` requires `modality_reliability`, `reliability_bias_norm`, and `corruption_response`.
...
Robustness experiment configs must declare at least one corruption or missing-modality setting for every required Step 6 stress family before launch, so incomplete coverage is rejected at configuration time rather than discovered only after evaluation.

Region-text public entry requires passed stackability, CATO collapse, and CATO alignment diagnostics gates from the controlled report. Region-text public gates must include CATO top alignment accuracy, grounding accuracy improvement paired with lower CATO alignment entropy, and RCEO-supported router-load shift under corrupted or missing visual-region settings.

Sentiment/emotion public entry requires passed SPO/TANSO primary diagnostics, RCEO gate, LRIO admission/non-interference evidence, and no-RCEO ablation degradation evidence. Sentiment/emotion public gates must include clean-setting LRIO rank entropy for exploratory LRIO rows, SPO prototype entropy, SPO top-prototype differentiation across emotion classes, and RCEO reliability calibration with expected calibration error evidence. Router-load or ablation deltas alone are not enough to claim LRIO/SPO/RCEO validity.
